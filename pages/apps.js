const appsAnimation = false
const appsRun = 'loadAppPage()'
const appsRunPost = null
const appsTemplate = `
	<div class="breadcrumb">
<a onclick="template('home');return false" href="javascript:void(0)">Home</a>&nbsp>&nbspApps
	</div>
	<br>
	<br>
	<link rel="stylesheet" href="pages/apps.css">
	<link rel="stylesheet" href="apps/icons.css">
	<br><div id="page-title" style="text-align: center;">APPS (bad) (不好)</div>
	<br><div style="text-align: center">"^"^"^"^"^"^"^ * ^"^"^"^"^"^"^"</div>
	<br><img src="/assets/images/maomao.gif" style="width: 300px; margin-left: calc((100% - 300px) / 2)">
`
